...
function renderPaymentList() {
    if (!savedPayments.length) {
        paymentList.innerHTML = '<p class="hint">Nenhum pagamento registrado. Aguarde confirmações assistidas.</p>';
        return;
    }

    paymentList.innerHTML = savedPayments.map((payment, index) => {
        return `
...
function renderChat() {
    if (!chatMessages.length) {
        chatWindow.innerHTML = '<p>Inicie a conversa com o inquilino usando o formulário abaixo.</p>';
        return;
    }
...
function renderAppointment() {
    ...
    scheduleDisplay.textContent = `${appointment.date} às ${appointment.time}` + (appointment.note ? ` — ${appointment.note}` : '');
}
...
paymentList.addEventListener('click', (event) => {
    ...
    localStorage.setItem('lastPaymentConfirmed', JSON.stringify({
    ...
    showToast(`Pagamento de ${confirmedPayment.tenant} confirmado.`, () => {
        ...
        savedPayments.splice(lastConfirmedPayment.index, 0, lastConfirmedPayment.payment);
        savePayments();
        renderPaymentList();
...
if (!savedPayments.length) {
    const samplePayments = [
        { tenant: 'Apartamento 201', amount: 'R$ 1.680,00', due: '07/06', status: 'Pendente, confirme no banco' },
        { tenant: 'Apartamento 110', amount: 'R$ 1.320,00', due: '10/06', status: 'Em análise' },
        { tenant: 'Apartamento 304', amount: 'R$ 1.940,00', due: '12/06', status: 'Pendente, aguardando comprovante' }
    ];
...
if (paymentMethodButton) {
    paymentMethodButton.addEventListener('click', () => {
        const method = prompt('Digite o meio de pagamento fixo (ex: Pix, QR ou chave):', 'Pix');
        if (!method) {
            return;
        }

        alert(`Método de pagamento fixado: ${method}. Atualize os dados de cobrança conforme necessário.`);
    });
}
...
if (ownerChatForm) {
    ownerChatForm.addEventListener('submit', (event) => {
        ...
        const messageText = document.getElementById('owner-chat-message').value.trim();
        if (!messageText) {
            return;
        }
